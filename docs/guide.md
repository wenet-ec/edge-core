...
**Enrollment key.** A short-lived credential a fresh agent uses to join a specific cluster. You generate the key on the admin, configure it as `ENROLLMENT_KEY` on the agent, and the agent enrolls itself on first boot. Keys are cluster-scoped — one key per cluster.
...
**Command execution.** The actual unit of work. **One command fans out into one execution per targeted node.** Each execution has its own status (`pending`, `sent`, `completed`, `cancelled`, `expired`, `dropped`), output, exit code, and timing. `completed` is the reported success/failure status — read `exit_code` to distinguish. `dropped` means its target node was deleted. A cancelled or expired execution is finalized after the Agent reports its exit code; completed and dropped executions are finalized immediately. When you "run a command on 50 nodes," you create 1 command and the system creates 50 executions.
...
- If `AGENT_PROXY_AUTH_ENABLED=false`, the agent accepts any credentials and effectively behaves as a transparent local proxy
...
**The agent is technically a standalone binary** — it's an OTP release that runs as a supervision tree — but its supported deployment is the published Linux container. The image bundles the complete host-management runtime: the agent process, netclient (WireGuard), an SSH server, Prometheus exporters, forward proxies, SQLite, diagnostics, and their supervision wiring.

The takeaway: when you deploy an agent, you are deploying a complete Linux host-management runtime in a container, not merely an Elixir application.
