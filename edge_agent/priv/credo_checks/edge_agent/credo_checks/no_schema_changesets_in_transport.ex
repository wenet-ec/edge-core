# edge_agent/priv/credo_checks/edge_agent/credo_checks/no_schema_changesets_in_transport.ex
defmodule EdgeAgent.CredoChecks.NoSchemaChangesetsInTransport do
  @moduledoc false
  ...
  use Credo.Check,
    ...
    explanations: [
      ...
      Controllers should call the domain context. The context owns the Form ->
      Checks -> Resource/Persistence -> Schema changeset -> DB constraint
      pipeline.
      """
    ]
  ...
  defp walk({{:., meta, [{:__aliases__, _alias_meta, module_parts}, function]}, _call_meta, _args} = ast, ctx)
    ...
    issue = issue_for(ctx, meta, "#{Name.full(module_parts)}.#{function}")
