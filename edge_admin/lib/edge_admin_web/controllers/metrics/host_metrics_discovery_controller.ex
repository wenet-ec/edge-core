...
defmodule EdgeAdminWeb.Controllers.Metrics.HostMetricsDiscoveryController do
  ...
  operation(:index,
    ...
    description:
      "Service discovery endpoint for Prometheus HTTP SD (host metrics). Returns every node matching the list-node filters in the format expected by http_sd_configs. The response is a complete, unpaginated target snapshot; without status__in, it includes healthy, unhealthy, and unreachable nodes.",
        ...
        Enum.map(nodes, fn node ->
          %{
            ...
            labels: %{
              ...
              __metrics_path__: "/api/v1/nodes/#{node.id}/metrics/host/raw",
              node_id: node.id
            }
          }
        end)
