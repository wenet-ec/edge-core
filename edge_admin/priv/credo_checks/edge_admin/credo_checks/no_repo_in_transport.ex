# edge_admin/priv/credo_checks/edge_admin/credo_checks/no_repo_in_transport.ex
defmodule EdgeAdmin.CredoChecks.NoRepoInTransport do
  @moduledoc false
  ...
  use Credo.Check,
    ...
    explanations: [
      ...
      the validation and persistence pipeline.
      """
    ]
  ...
  @message "Transport modules must not call `EdgeAdmin.Repo` directly. Call the domain context instead."
  ...
  defp walk({:__aliases__, meta, module_parts} = ast, ctx) do
    issue =
      if Name.full(module_parts) == "EdgeAdmin.Repo" do
        issue_for(ctx, meta, "EdgeAdmin.Repo")
      end
  ...
  defp walk({:alias, _meta, [{{_, _, [{:__aliases__, _opts, base_alias}, :{}]}, _, aliases}]} = ast, ctx) do
    issues =
      Enum.flat_map(aliases, fn {:__aliases__, meta, module} ->
        ...
        if full_module == "EdgeAdmin.Repo" do
