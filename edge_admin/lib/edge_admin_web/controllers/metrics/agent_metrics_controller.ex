...
defmodule EdgeAdminWeb.Controllers.Metrics.AgentMetricsController do
  ...
  operation(:show,
    summary: "Raw agent metrics proxy",
    description: "Scrapes Prometheus metrics from a node's PromEx endpoint via Gateway and returns raw text format.",
    parameters: [
      node_id: [
        in: :path,
  ...
  def show(conn, %{node_id: node_id}) do
    with {:ok, metrics_text} <- Metrics.scrape_agent_metrics(node_id) do
      conn
