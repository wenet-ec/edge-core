# edge_agent/priv/credo_checks/edge_agent/credo_checks/no_nexmaker_outside_vpn_boundary.ex
defmodule EdgeAgent.CredoChecks.NoNexmakerOutsideVpnBoundary do
  @moduledoc false
  ...
  @agent_app_path "lib/edge_agent/"
  @allowed_paths [
    "lib/edge_agent/vpn/"
  ]

  @message "Direct `Nexmaker` usage is only allowed at the Agent VPN boundary. Call `EdgeAgent.Vpn` instead."

  @impl Credo.Check
  def run(%SourceFile{} = source_file, params) do
    if agent_app_file?(source_file.filename) and not allowed_file?(source_file.filename) do
  ...
  defp agent_app_file?(filename), do: path_matches?(filename, @agent_app_path)
