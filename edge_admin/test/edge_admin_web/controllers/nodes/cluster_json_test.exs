...
defmodule EdgeAdminWeb.Controllers.Nodes.ClusterJSONTest do
  ...
  describe "show/1" do
    test "wraps cluster in %{data: ...}" do
      result = ClusterJSON.show(%{cluster: fake_cluster()})
    ...
    test "node_count is 0 when nodes is empty list" do
      data = ClusterJSON.show(%{cluster: fake_cluster(%{nodes: []})}).data
    ...
    test "node_count matches the number of nodes in the list" do
      ...
      data = ClusterJSON.show(%{cluster: fake_cluster(%{nodes: nodes})}).data
  ...
  describe "index/1 — data array" do
    test "result has :data and :pagination keys" do
      result = ClusterJSON.index(%{clusters: [], meta: fake_meta()})
      assert Map.has_key?(result, :data)
    ...
    test "each cluster is rendered" do
      ...
      result = ClusterJSON.index(%{clusters: [cluster], meta: fake_meta()})
    ...
    test "multiple clusters rendered in order" do
      ...
      result = ClusterJSON.index(%{clusters: clusters, meta: fake_meta()})
  ...
  describe "index/1 — pagination field renames from Flop.Meta" do
    test "current_page is renamed to page" do
      result = ClusterJSON.index(%{clusters: [], meta: fake_meta(current_page: 3)})
    ...
    test "total_count is renamed to total" do
      result = ClusterJSON.index(%{clusters: [], meta: fake_meta(total_count: 55)})
    ...
    test "has_next_page? is renamed to has_next" do
      result = ClusterJSON.index(%{clusters: [], meta: fake_meta(has_next_page?: true)})
    ...
    test "has_previous_page? is renamed to has_prev" do
      result = ClusterJSON.index(%{clusters: [], meta: fake_meta(has_previous_page?: true)})
    ...
    test "page_size is passed through unchanged" do
      result = ClusterJSON.index(%{clusters: [], meta: fake_meta(page_size: 50)})
      assert result.pagination.page_size == 50
    end

    test "total_pages is passed through unchanged" do
      result = ClusterJSON.index(%{clusters: [], meta: fake_meta(total_pages: 7)})
      assert result.pagination.total_pages == 7
    end

    test "has_next false is preserved" do
      result = ClusterJSON.index(%{clusters: [], meta: fake_meta(has_next_page?: false)})
      assert result.pagination.has_next == false
    end

    test "has_prev false is preserved" do
      result = ClusterJSON.index(%{clusters: [], meta: fake_meta(has_previous_page?: false)})
