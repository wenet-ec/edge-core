# edge_admin/lib/edge_admin/proxy_servers/transport/forwarder.ex
defmodule EdgeAdmin.ProxyServers.Transport.Forwarder do
  @moduledoc """
  ...
  Deadlines:
    ...
    * Drain grace: if the coordinator receives `{:drain, grace_ms}`, a new
      deadline is set at `now + grace_ms` and the tunnel is torn down when it
      fires (typically earlier than the total timeout).

  Byte counters are accumulated per direction and emitted as
  `[:edge_admin, :proxy, :tunnel, :closed]` telemetry when both loops finish.
  """
  ...
  Transfers `client_socket` ownership to the client→target forwarder; callers
  must not `recv` on it after this returns.

  Blocks until both directions complete or a deadline fires. The caller process
  (the tunnel handler) may receive `{:drain, grace_ms}` while blocked — when
  that happens the forwarder finishes in-flight work within the grace window.
  """
  ...
  defp do_wait(client_socket, target_socket, up, down, state, start, deadline, metadata) do
    ...
    receive do
      ...
      {:drain, grace_ms} ->
        # Tighten the deadline. Loops will see it via shrinking recv timeouts.
        new_deadline = min(deadline, System.monotonic_time(:millisecond) + grace_ms)

        # Also shut down client→target write side to signal the upstream that
        # no more data is coming — bounded end for long-idle tunnels.
  ...
  defp emit_closed(bytes_up, bytes_down, duration_ms, metadata) do
    :telemetry.execute(
      [:edge_admin, :proxy, :tunnel, :closed],
  ...
  defp run(src, dst, coordinator, deadline) do
    ...
    # close it, potentially cutting off the opposite direction that still
    # needs to write to `src` as its own destination.
    _ = dst
