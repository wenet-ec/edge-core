...
defmodule EdgeAdmin.Nodes.Schemas.Cluster do
  ...
  @doc """
  Returns the public-facing map for this cluster — the canonical shape
  both REST and MCP serialize. Includes a nested `nodes` array with each
  node's identity + VPN hostname. Requires `nodes` to be preloaded.
  """
  @spec to_public(t()) :: map()
  def to_public(%__MODULE__{nodes: nodes} = cluster) do
    %{
      id: cluster.id,
      name: cluster.name,
