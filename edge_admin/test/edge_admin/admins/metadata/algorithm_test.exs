...
defmodule EdgeAdmin.Admins.Metadata.AlgorithmTest do
  ...
  describe "compute_assignments/2" do
    ...
    test "deterministic - cluster input order does not affect assignment" do
      ...
      admins = admins(a1: 100, a2: 100)
      c1 = %{name: "c1", nodes: ~w[n1 n2]}
      c2 = %{name: "c2", nodes: ~w[n3 n4]}
    ...
    test "tie-breaking uses stable hash affinity before admin name" do
      # Both admins have identical capacity and zero clusters — pure tie.
      # cluster-test hashes lower for admin-b than admin-a, so hash affinity
      # beats alphabetical order here. admin name remains only the final fallback.
      admins = admins("admin-b": 100, "admin-a": 100)
      clusters = clusters("cluster-test": ~w[n1])

      result = Algorithm.compute_assignments(admins, clusters)

      assert Map.has_key?(result.edge_clusters["admin-b"], "cluster-test")
      refute Map.has_key?(result.edge_clusters["admin-a"], "cluster-test")
    end
  ...
  describe "hash-affinity placement" do
    test "stable topology recomputes identically from shared inputs" do
      ...
      r2 = Algorithm.compute_assignments(admins, cluster_list)
      r3 = Algorithm.compute_assignments(admins, Enum.reverse(cluster_list))

      assert r2.edge_clusters == r1.edge_clusters
      assert r2.node_index == r1.node_index
      assert r3.edge_clusters == r1.edge_clusters
    end

    test "hash affinity does not override load balance" do
      ...
      owners =
        result.edge_clusters
        |> Enum.flat_map(fn {admin, clusters} ->
          Enum.map(clusters, fn {cluster, _nodes} -> {cluster, admin} end)
        end)
        |> Map.new()

      assert owners["c1"] != owners["c2"]
    end

    test "hash affinity does not override capacity" do
  ...
  describe "topology change behavior" do
    ...
    test "adding an admin gives it a fair share while preserving deterministic output" do
      ...
      after_join = Algorithm.compute_assignments(new_admins, cluster_list)
      after_join_reversed = Algorithm.compute_assignments(new_admins, Enum.reverse(cluster_list))

      assert after_join_reversed.edge_clusters == after_join.edge_clusters

      cluster_counts =
        after_join.edge_clusters
        |> Map.values()
        |> Enum.map(&map_size/1)
        |> Enum.sort()

      assert cluster_counts == [8, 8, 8, 8, 8]
    end

    test "removing an admin redistributes clusters deterministically across survivors" do
      ...
      after_removal = Algorithm.compute_assignments(new_admins, cluster_list)
      after_removal_reversed = Algorithm.compute_assignments(new_admins, Enum.reverse(cluster_list))

      moves = count_moves(stable, after_removal)
      assert moves >= a3_count, "must reassign at least a3's clusters, got #{moves} < #{a3_count}"
      assert after_removal_reversed.edge_clusters == after_removal.edge_clusters
      ...
      cluster_counts =
        after_removal.edge_clusters
        |> Map.values()
        |> Enum.map(&map_size/1)
        |> Enum.sort()

      assert cluster_counts == [4, 5]
    end
    ...
    test "after admin death, recomputation distributes orphaned clusters and stays stable" do
      ...
      after_death = Algorithm.compute_assignments(survivors, cluster_list)

      # Verify all clusters are assigned to surviving admins.
      assigned = after_death.edge_clusters |> Map.values() |> Enum.flat_map(&Map.keys/1)
      assert length(assigned) == 12

      # Verify stability: re-running from the same shared inputs changes nothing.
      followup = Algorithm.compute_assignments(survivors, cluster_list)
