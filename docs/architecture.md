...
Five adapters: `nats` (NATS — pub/sub by default, set `EVENT_BROKER_NATS_JETSTREAM=true` for durable log), `kafka` (any Kafka-compatible broker — Redpanda, Kafka, Confluent Cloud, etc.), `rabbitmq` (topic exchange, routing key = event type), `redis` (fire-and-forget pub/sub, no persistence), and `mqtt` (any MQTT 3.1.1 / 5 broker — EMQX, Mosquitto, HiveMQ, AWS IoT Core, etc.; configurable QoS 0/1/2). Pick the broker that matches your semantics — there is no recommended default.

The `type` field in the envelope doubles as the NATS subject, RabbitMQ routing key, and Redis channel (`edge.node.status_changed`, `edge.execution.completed`, etc.) — no parsing needed for broker-level filtering. The MQTT adapter rewrites `.` to `/` (`edge/node/status_changed`) so MQTT segment wildcards (`edge/#`, `edge/node/+`) work as expected.
