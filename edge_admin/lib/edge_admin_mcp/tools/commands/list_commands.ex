...
defmodule EdgeAdminMcp.Tools.Commands.ListCommands do
  ...
  - `has_expires_at` — true: commands with an expiration; false: commands without
  - `expires_at_gte` / `expires_at_lte` — expiration datetime range (ISO8601)
  ...
  ## Sorting
  - `order_by` — comma-separated fields: `timeout`, `expires_at`, `inserted_at`, `updated_at`
  ...
  schema do
    ...
    field :has_timeout, {:enum, ["true", "false"]}
    field :timeout_gte, :integer, min: 1
    field :timeout_lte, :integer, min: 1
    field :has_expires_at, {:enum, ["true", "false"]}
