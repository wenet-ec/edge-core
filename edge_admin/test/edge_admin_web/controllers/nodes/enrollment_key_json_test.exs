...
defmodule EdgeAdminWeb.Controllers.Nodes.EnrollmentKeyJSONTest do
  ...
  describe "show/1" do
    test "wraps data under :data key" do
      result = EnrollmentKeyJSON.show(%{enrollment_key: fake_key()})
      assert Map.has_key?(result, :data)
    end

    test "all 8 expected fields are present" do
      data = EnrollmentKeyJSON.show(%{enrollment_key: fake_key()}).data
    ...
    test "expired_at is nil when unset" do
      data = EnrollmentKeyJSON.show(%{enrollment_key: fake_key(%{expired_at: nil})}).data
      assert data.expired_at == nil
    end

    test "expired_at is passed through when set" do
      dt = ~U[2027-01-01 00:00:00Z]
      data = EnrollmentKeyJSON.show(%{enrollment_key: fake_key(%{expired_at: dt})}).data
      assert data.expired_at == dt
    end

    test "last_used_at is nil when unset" do
      data = EnrollmentKeyJSON.show(%{enrollment_key: fake_key(%{last_used_at: nil})}).data
      assert data.last_used_at == nil
    end

    test "last_used_at is passed through when set" do
      data = EnrollmentKeyJSON.show(%{enrollment_key: fake_key(%{last_used_at: @now})}).data
  ...
  describe "index/1 — data array" do
    test "empty key list returns empty data array" do
      result = EnrollmentKeyJSON.index(%{enrollment_keys: [], meta: fake_meta(total_count: 0)})
      assert result.data == []
    end

    test "single key returns data array with one element" do
      result = EnrollmentKeyJSON.index(%{enrollment_keys: [fake_key()], meta: fake_meta()})
    ...
    test "multiple keys returned in order" do
      ...
      result =
        EnrollmentKeyJSON.index(%{enrollment_keys: [k1, k2], meta: fake_meta(total_count: 2)})
  ...
  describe "index/1 — pagination shape" do
    setup do
      ...
      result = EnrollmentKeyJSON.index(%{enrollment_keys: [], meta: meta})
