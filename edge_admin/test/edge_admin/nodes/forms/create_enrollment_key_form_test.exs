...
defmodule EdgeAdmin.Nodes.Forms.CreateEnrollmentKeyFormTest do
  ...
  describe "changeset/1 — valid cases" do
    ...
    test "name is accepted when present" do
      assert {:ok, result} = CreateEnrollmentKeyForm.changeset(%{name: "prod rollout"})
      assert result["name"] == "prod rollout"
    end

    test "name as null is preserved (explicit null)" do
      assert {:ok, result} = CreateEnrollmentKeyForm.changeset(%{name: nil})
      assert Map.has_key?(result, "name")
      assert result["name"] == nil
    end

    test "absent name is excluded from result" do
      assert {:ok, result} = CreateEnrollmentKeyForm.changeset(%{})
  ...
  describe "changeset/1 — invalid params" do
    test "non-map params return a base error" do
      assert {:error, changeset} = CreateEnrollmentKeyForm.changeset("bad")
      assert %{base: [msg]} = errors_on(changeset)
      assert msg =~ "expected a map"
    end

    test "nil params return a base error" do
      assert {:error, changeset} = CreateEnrollmentKeyForm.changeset(nil)
      assert %{base: [msg]} = errors_on(changeset)
      assert msg =~ "expected a map"
    end
  end
