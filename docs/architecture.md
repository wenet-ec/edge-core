...
- Assignment is computed from scratch each time: clusters are sorted by size descending, then name ascending, and each cluster is assigned to the best available admin scored by (fewest clusters managed, then highest remaining capacity, then deterministic hash affinity for `{cluster_name, admin_name}`, then admin ID as final tiebreaker). Hash affinity keeps exact ties stable and avoids admin-name bias without overriding load balance or capacity
...
- **Encryption at rest** for `secret` and `headers`. `ENCRYPTION_KEY` and `ENCRYPTION_TAG` are required at admin boot. Rotation is supported via `EdgeAdmin.Release.rotate_encryption_key/0` (see [`examples/operations/rotate_encryption_key.yml`](https://github.com/wenet-ec/edge-core/blob/main/examples/operations/rotate_encryption_key.yml)).
...
- **Streamable HTTP transport.** Standard MCP. A single `/mcp` endpoint accepts an opaque bearer token (`MCP_KEY` or `MASTER_KEY`) when authentication is enabled; missing credentials can still reach the protocol so explicitly public tools such as `create_public_enrollment_key` can be used without a second endpoint.
- **Tools mirror the REST API surface.** Every REST operation has an equivalent MCP tool. The catalog is explicitly registered in `EdgeAdminMcp.ToolRegistry`, not auto-generated from controllers. Adding a REST endpoint does not automatically expose it via MCP — write the matching tool module under `edge_admin_mcp/tools/<domain>/` and register it in the registry. Each registration belongs to a named scope (`:authenticated` or `:public`) and can declare degraded-mode behavior. Discovery remains dynamic for clients via standard `tools/list`.
...
| Admin ↔ Admin (Erlang)   | Shared `ADMIN_CLUSTER_COOKIE` + connection verified against PostgreSQL + Netmaker |
