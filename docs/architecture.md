...
Adapters cover: `nats` (NATS — pub/sub by default, set `EVENT_BROKER_NATS_JETSTREAM=true` for durable log), `kafka` (any Kafka-compatible broker — Redpanda, Kafka, Confluent Cloud, Azure Event Hubs, etc.), `rabbitmq` (topic exchange, routing key = event type), `redis` (fire-and-forget pub/sub, no persistence), `mqtt` (any MQTT 3.1.1 / 5 broker — EMQX, Mosquitto, HiveMQ, AWS IoT Core, Azure Event Grid in MQTT broker mode, etc.; configurable QoS 0/1/2), `aws_sns` (managed AWS service — pre-provisioned topics by domain, IAM-based auth via the standard AWS credential chain), and `google_pubsub` (managed GCP service — pre-provisioned topics by domain, OAuth2 auth via the standard GCP credential chain). Pick the broker that matches your semantics — there is no recommended default.

The `type` field in the envelope doubles as the NATS subject, RabbitMQ routing key, and Redis channel (`edge.node.status_changed`, `edge.execution.completed`, etc.) — no parsing needed for broker-level filtering. The MQTT adapter rewrites `.` to `/` (`edge/node/status_changed`) so MQTT segment wildcards (`edge/#`, `edge/node/+`) work as expected. The AWS SNS and Google Cloud Pub/Sub adapters promote `type` and `corename` to message attributes so subscription filter policies (SNS) or filter expressions (Pub/Sub) can match without parsing the body — neither service supports topic-name wildcards.
