...
defmodule EdgeAdminMcp.Tools.Nodes.ChangeNodeCluster do
  ...
  - `node_id` — required. The node to move.
  - `cluster_name` — required. The destination cluster.

  The returned node reflects the saved assignment immediately; network
  connectivity may briefly reflect the previous assignment while it converges.
  Moving the node clears its recovery key and cluster-specific aliases. Treat
  this as a destructive operation.
  """
