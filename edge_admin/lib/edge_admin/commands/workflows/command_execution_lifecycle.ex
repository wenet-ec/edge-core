...
defmodule EdgeAdmin.Commands.Workflows.CommandExecutionLifecycle do
  ...
  def drop_node_command_executions(node_id, cluster_name) do
    ...
    executions =
      Repo.all(
        from(ce in CommandExecution,
          ...
          where: ce.status in ^active_statuses,
          preload: [:command]
        )
      )
    ...
    dropped =
      ...
      |> Enum.reduce([], fn execution, acc ->
        case transition_status(execution, active_statuses, status: :dropped) do
  ...
  acknowledgement. The agent is the source of truth for the reported result: an
  ...
  - `params` - Map with:
    ...
    - `"completed_at"` (optional) — accepted for wire compatibility but ignored;
      Admin generates the authoritative timestamp
  ...
  def update_command_execution_result(execution, params) do
    with :ok <- Checks.CommandExecutionAcceptsResultCheck.check(execution),
      ...
      # `CommandExecutionAcceptsResultCheck` (cancelled/expired with nil Admin-owned
      # `completed_at`) is
      # encoded directly in the WHERE clause so two concurrent reports cannot
      # both succeed and clobber each other.
      result = transition_to_result(execution, build_result_set(attrs, result_status))
      ...
      case result do
        {:ok, updated_execution} ->
          ...
          event_type = if result_status == :cancelled, do: :cancelled, else: :completed
      ...
      case result do
        {:error, :stale_state} ->
          # The row changed before the conditional update. Treat this as a
          # conflict because it no longer accepts an Agent result.
  ...
  # `completed_at` is generated by Admin when the form normalizes the result,
  # rather than trusting the Agent's wall clock.
  # `output`/`exit_code` are either present-with-a-value or absent.
  defp build_result_set(attrs, result_status) do
    base = [
    ...
    |> maybe_put(
      :cancelled_at,
      result_status == :cancelled && DateTime.truncate(DateTime.utc_now(), :second)
    )
  end
    ...
    with :ok <- Checks.CommandExecutionCancellableCheck.check(execution) do
      case execution.status do
        :pending ->
          ...
          # result back) can clobber a finalized row back to :sent / :expired. See the
  ...
  # completed_at IS NULL (race-window placeholders).
  #
