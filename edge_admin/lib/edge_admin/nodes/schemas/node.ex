...
defmodule EdgeAdmin.Nodes.Schemas.Node do
  ...
  @doc """
  Returns the public-facing map for this node — the canonical shape both
  REST and MCP serialize. Includes computed hostnames + nested aliases
  with their per-alias `vpn_hostname`. Requires `cluster` and `aliases`
  to be preloaded.
  """
  @spec to_public(t()) :: map()
  def to_public(%__MODULE__{cluster: cluster, aliases: aliases} = node) do
    %{
      id: node.id,
      ...
      status: node.status,
      vpn_hostname: vpn_hostname(node),
      mdns_hostname: mdns_hostname(node),
