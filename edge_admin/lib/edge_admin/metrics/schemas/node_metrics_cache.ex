...
defmodule EdgeAdmin.Metrics.Schemas.NodeMetricsCache do
  ...
  The metrics context filters out anything older than the fixed 5-minute window
  at read time. Subsequent pushes overwrite the row in place via the
  `(node_id, metrics_type)` upsert, so each node holds at most one row per
  metrics type for the lifetime of the node.
  """
