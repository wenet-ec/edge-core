...
defmodule EdgeAdmin.Release do
  ...
  to invoke repeatedly where the operation is idempotent.
  """
  ...
  This task is idempotent and optional. It skips when no default cluster name
  is configured or when the cluster already exists. Missing address ranges are
  generated by the normal cluster creation flow.
  ...
  Idempotent — safe to run multiple times. If any required rotation setting is
  missing, it skips without touching the database. When all are present, every
  registered encrypted field is re-encrypted from the old key to the new key.

  Required env vars (all four, or none):
    - `ROTATE_OLD_ENCRYPTION_KEY`  — old key, base64-encoded 32 bytes
    - `ROTATE_OLD_ENCRYPTION_TAG`  — old cipher tag
    - `ROTATE_NEW_ENCRYPTION_KEY`  — new key, base64-encoded 32 bytes
    - `ROTATE_NEW_ENCRYPTION_TAG`  — new cipher tag
