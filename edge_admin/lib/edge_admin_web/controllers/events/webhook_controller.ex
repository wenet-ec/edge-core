...
defmodule EdgeAdminWeb.Controllers.Events.WebhookController do
  ...
  operation(:delete,
    ...
    description:
      "Permanently delete a webhook. Webhooks are immutable after create — to change any field (URL, secret, headers, subscribed_events) delete and recreate. The retry budget is deployment-wide, not per-webhook.",
    parameters: [PathParams.uuid(:id, "Webhook ID")],
    responses: %{
