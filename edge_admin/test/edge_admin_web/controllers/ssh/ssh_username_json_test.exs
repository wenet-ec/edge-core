...
defmodule EdgeAdminWeb.Controllers.Ssh.SshUsernameJSONTest do
  ...
  describe "show/1" do
    test "wraps username in %{data: ...}" do
      result = SshUsernameJSON.show(%{ssh_username: fake_username()})
      assert Map.has_key?(result, :data)
    end

    test "data contains all required fields" do
      data = SshUsernameJSON.show(%{ssh_username: fake_username()}).data
    ...
    test "scalar fields are passed through correctly" do
      ...
      data = SshUsernameJSON.show(%{ssh_username: username}).data
  ...
  describe "show/1 — public_keys" do
    test "public_keys is empty list when username has no keys" do
      data = SshUsernameJSON.show(%{ssh_username: fake_username(%{ssh_public_keys: []})}).data
  ...
  describe "index/1 — data array" do
    test "result has :data and :pagination keys" do
      result = SshUsernameJSON.index(%{ssh_usernames: [], meta: fake_meta()})
      assert Map.has_key?(result, :data)
    ...
    test "each username is rendered with has_password delegation" do
      ...
      result = SshUsernameJSON.index(%{ssh_usernames: [username], meta: fake_meta()})
    ...
    test "multiple usernames rendered in order" do
      ...
      result = SshUsernameJSON.index(%{ssh_usernames: [u1, u2], meta: fake_meta()})
  ...
  describe "index/1 — pagination field renames from Flop.Meta" do
    test "current_page is renamed to page" do
      result = SshUsernameJSON.index(%{ssh_usernames: [], meta: fake_meta(current_page: 2)})
    ...
    test "total_count is renamed to total" do
      result = SshUsernameJSON.index(%{ssh_usernames: [], meta: fake_meta(total_count: 42)})
    ...
    test "has_next_page? is renamed to has_next" do
      result = SshUsernameJSON.index(%{ssh_usernames: [], meta: fake_meta(has_next_page?: true)})
    ...
    test "has_previous_page? is renamed to has_prev" do
      result = SshUsernameJSON.index(%{ssh_usernames: [], meta: fake_meta(has_previous_page?: true)})
    ...
    test "page_size is passed through unchanged" do
      result = SshUsernameJSON.index(%{ssh_usernames: [], meta: fake_meta(page_size: 50)})
      assert result.pagination.page_size == 50
    end

    test "total_pages is passed through unchanged" do
      result = SshUsernameJSON.index(%{ssh_usernames: [], meta: fake_meta(total_pages: 3)})
      assert result.pagination.total_pages == 3
    end

    test "has_next false is preserved" do
      result = SshUsernameJSON.index(%{ssh_usernames: [], meta: fake_meta(has_next_page?: false)})
      assert result.pagination.has_next == false
    end

    test "has_prev false is preserved" do
      result = SshUsernameJSON.index(%{ssh_usernames: [], meta: fake_meta(has_previous_page?: false)})
