...
- **Streamable HTTP transport.** Standard MCP. A single `/mcp` endpoint accepts an opaque bearer token (`MCP_KEY` or `MASTER_KEY`) when authentication is enabled; missing credentials can still reach the protocol so public tools can be exposed later without a second endpoint.
